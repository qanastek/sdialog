...
class BaseOrchestrator(ABC):
    ...
    Methods:
        __call__: Returns an instruction or action for the agent.
        is_persistent: Indicates if the instruction/action should persist across turns.
        get_event_label: Returns a label for the event generated by this orchestrator.
        reset: Resets the orchestrator's internal state.
        json: Serializes the orchestrator configuration.
    """
    _target = None
    _event_label = None
    _persistent = False
...
class SimpleReflexOrchestrator(BaseOrchestrator):
    ...
    Attributes:
        condition (callable): A function that takes an utterance and returns True if the instruction should be applied.
        instruction (str): The instruction to be applied if the condition is met.
    """
...
class LengthOrchestrator(BaseOrchestrator):
    ...
    Attributes:
        min (int): The minimum number of turns before the dialogue should be considered complete.
        max (int): The maximum number of turns allowed for the dialogue.
    """
...
class ChangeMindOrchestrator(BaseOrchestrator):
    ...
    Attributes:
        probability (float): The probability of the mind change instruction being applied.
        reasons (Union[str, List[str]]): The reasons for the mind change, can be a string or a list of strings.
        max_times (int): The maximum number of times the mind change instruction can be applied.
    """
...
class SimpleResponseOrchestrator(BaseOrchestrator):
    ...
    Attributes:
        responses (List[Union[str, Dict[str, str]]]): A list of possible responses, can be strings or dictionaries mapping actions to responses.
        graph (Dict[str, str]): A graph mapping actions to next actions for controlling dialogue flow.
        sbert_model (str): The sentence-BERT model used for encoding responses.
        top_k (int): The number of top similar responses to consider.
    """
...
class InstructionListOrchestrator(BaseOrchestrator):
    ...
    Attributes:
        instructions (List[Union[str, Dict[int, str]]]): A list of instructions or a turn-number:instruction dictionary to be provided to the agent.
    """
